...
public class KeywordPanel1 extends JPanel implements MouseListener,
	...
	public KeywordPanel1(XPanel mainPanel, JTextArea commentArea, XButton[] xb) {
	...
	public KeywordPanel1(CommentBox cb) {
		this.cb = cb;
	}
	
	
	public KeywordPanel1() {
	...
	private XButton closeMpButton = null;
...
//		mainPanel.add(showMainPanelButton);
		...
		mainPanel.setBounds(810, 90, 145, 500);
...
//		this.hideMainPanel();
	...
	public void init() {
		...
		// heading.setBackground(Color.white);
//		heading.setAlignment(Label.CENTER);
		heading.setBounds(0, 0, 130, 20);
		heading.setBackground(new Color(220,220,220));
		
		closeMpButton = new XButton();
		closeMpButton.setText("X");
		closeMpButton.setLabel("X");
		closeMpButton.setBounds(130,0,20,20);
		closeMpButton.setBackground(buttonColor);
		closeMpButton.setForeground(Color.WHITE);
		closeMpButton.setFont(buttonFont);
		closeMpButton.addActionListener(this);
...
//		this.hideMainPanel();
	...
	public void setSelectionForegroundColor(Color c) {
	...
	public void selectComment(String comment) {
		...
		if (currentSelected != -1
			...
			listPanel.getComponent(currentSelected).setForeground(
					Color.BLACK);
		}
		setEditable(false);
		for (int i = 0; i < cm.getNumChildren(); i++) {
			...
			//	commentArea.setText(((XModel) cm.get(i).get("text")).get()
				...
				cb.setComment(cm.get(i));
				cb.setKeywordPhysician(getKeywordPhysician());
				cb.displayComment(((XModel) cm.get(i).get("text")).get()
						.toString());
				
//				cb.setVisible(true);
				listPanel.getComponent(i).setBackground(color);
				listPanel.getComponent(i).setForeground(foregroundColor);
...
//		cb.setKeywordPhysician(getKeywordPhysician());
//		cb.displayComment(((JTextArea) arg0.getSource()).getText());
...
//			this.confirmDelete();
	...
	public void setKeywordStage(String stage) {
		cb.setKeywordStage(stage);
	}
}
